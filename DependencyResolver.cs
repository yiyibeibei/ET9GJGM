...
namespace Hibzz.DependencyResolver
{
    ...
    public class DependencyResolver
    {
        ...
        static void MoveToPackage(string package, string version)
        {
            ...
            if (targetDir == null)
            {
                targetDir = allDirs.FirstOrDefault(dir => 
            Path.GetFileName(dir).Contains(packageName) // 包含关键字
             || Path.GetFileName(dir).StartsWith(packageName + "@") // 或按前缀匹配
            );
            }

            if (targetDir == null)
            {
                Debug.LogError($"未找到包含 '{packageName}' 的包目录");
                return;
            }
            ...
            string dir = Path.Combine(packageCachePath, packageName);
